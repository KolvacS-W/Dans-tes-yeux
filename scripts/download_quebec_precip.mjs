...
const OUTPUT_PATH = path.join(process.cwd(), "data", `quebec_city_monthly_precip_${START_YEAR}_${END_YEAR}.json`);
...
function computeMonthlyTotals(times, values) {
  const accum = {};
  ...
  for (let i = 0; i < times.length; i++) {
    ...
    if (!accum[year][month]) accum[year][month] = 0;

    accum[year][month] += value;
  }
  ...
  for (const year of Object.keys(accum)) {
    ...
    for (const month of Object.keys(accum[year])) {
      monthly[year][month] = Number(accum[year][month].toFixed(4));
    }
  }
...
async function fetchCityMonthly(city) {
  ...
  return {
    ...
    monthly: computeMonthlyTotals(times, values)
  };
}
...
async function writePayload(rows) {
  const payload = {
    ...
    aggregate: "monthly_sum_from_daily_precipitation_sum",
    units: "mm",
